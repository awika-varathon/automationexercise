...
export const filledFormElementVauleBycriteria = (options) => {
    ...
    // Checking: If howToFilled is 'reference', check vaule first is same as reference value or not in case some input field is not disable which can change value.
    // Note: If input field is disable setting, howToFilled as 'autofilled'.
    if(howToFilled === 'reference') {
        // Check condition by form type . 
        switch (true) {
            case formType === 'input':
                ...
                // INPUT: Check with 1st defaultValue as reference value and set checkValue as reference value
    ...
    // Filled: Filled value by formType & howToFilled.
    if(criteria[name] === undefined || criteria[name] === '' || criteria[name] === checkValue) {
        // Not fill value. If criteria is undifined, is '', or criteria is same as checkValue.
        ...
        switch (true) {
            ...
            case formType === 'input' && (howToFilled === 'type' || howToFilled === 'reference'):
                // INPUT & TYPE: Filled input with criteria's value by type's value and click body to clear in case this input trigger other element.
                // e.g. element name = input[data-qa='name']
                cy.get(`input${elementName}`).clear().type(criteria[name]);
                cy.get('body').click(0,0);
                break;
            case formType === 'textarea':
                // TEXTAREA: Filled textarea with criteria's value by type's value and click body to clear in case this textarea trigger other element.
                // e.g. element name = input[data-qa='name']
                cy.get(`textarea${elementName}`).clear().type(criteria[name]);
                cy.get('body').click(0,0);
                break;
            case formType === 'input' && howToFilled === 'autofilled':
                // INPUT & AUTOFILLED: Only check input value is equal criteria's value.
                // e.g. element name = input[data-qa='name']
                cy.get(`input${elementName}`)
            ...
            case formType === 'dropdown':
                ...
                // Note: Set 'force: true' for dropdown that not visible which need to scroll down to see the option.
            ...
            case formType === 'upload': 
                // Upload: Click to upload file from folder 'cypress/fixtures/testCasecriteria/upload'
...
// Checking: Loop checkElement to check element's value equal criteria's value or not.
...
export const loopCheckElementAndCompareValues = ($ele, checkElement, criteria) => {
    ...
    checkElement.forEach(element => {
        ...
        // Get elementValue from id or class by element's type.
        let elementValue = '';
        switch(true) {
            ...
            case element['type'].includes('attr'):
                // If element's type includes 'attr', get attr's value with type as attr's name.
                // e.g. element['type'] = attr|title => title
                elementValue = $ele.find(element['id']).attr(element['type'].replace('attr|', ''));
                // console.log('Attr')
                break;
            case element['type'] === 'tr|id':
                // If element's type is 'tr|id', get id's value from tr which is $ele level to get this row product's id.
        ...
        // If element's toLowerCase is true, convert both value to lowercase before compare.
        if(element['toLowerCase']) {
            baseValue = baseValue.toLowerCase();
            elementValue = elementValue.toLowerCase();
        }

        // Compare baseValue with elementValue.
        cy.get($ele)
            .should(() => {
                // Check element's matchType.
                switch(element['matchType']) {
                    case 'equal':
                        // equal: Compare elementValue equal baseValue.
                        expect(elementValue).to.equal(baseValue, message);
                        break;
                    case 'contains':
                        // contains: Compare elementValue include in baseValue.
                        expect(elementValue).to.include(baseValue, message);
                        break;
                    case 'startsWith':
                        // startsWith: Compare elementValue starts with in baseValue.
                        expect(elementValue).to.match(new RegExp(`^${baseValue}`, 'g'), message);
                        break;
                    case 'endsWith':
                        // endsWith: Compare elementValue ends with in baseValue.
                        expect(elementValue).to.match(new RegExp(`${baseValue}$`, 'g'), message);
                        break;
                }
        });
...
// Text: Remove all text line breaks when get text from function text().
...
// Note: Using for test case type: Brands, Category, and Search.
...
export const getProductsObjectFromProductsConfig = (options) => {
    ...
    // e.g. productsConfig = [{ "id": "1", "name": "Blue Top", "price": "Rs. 500", "brand": "Polo", "category": { "usertype": { "usertype": "Women" }, "category": "Tops" }, "category-type": "Women > Tops", "img": "/get_product_picture/1", "url": "/product_details/1", "availability": "In Stock", "condition": "New" },...]
...
// InputConfig: Get input config to check element failed fill field.
...
export const convertJSONArrayToGoldenCSV = (options) => {
    ...
    cy.task("checkFileExist", filePathName).then((fileExist) => {
        ...
        if(isFirstTestCase) {
            ...
            cy.readFile(filePathName).then(csvRecords => {
