...
///////////////////////////////////////////
// ++++ API & Response ++++
// API: Set website all base intercepts.
Cypress.Commands.add('setWebsiteBaseIntercept', () => {
    // Get api info from apiConfig.json to loop set intercept.
...
Cypress.Commands.add('setOrderTestCasecriteriaVariableFromExcel', (options) => {
    ...
    cy.task('readXlsx', { file: filePathName, sheet: testCaseName}).then((rows) => {
        ...
        // Product: Get all products detail from 'productsConfig.json'
        ...
        rows.forEach((row, i) => {
            ...
            // Product: Find this product detail from productsConfig by name.
            // e.g. productDetail = { "id": "1", "name": "Blue Top", "price": "Rs. 500", "brand": "Polo", "category": { "usertype": { "usertype": "Women" }, "category": "Tops" }, "category-type": "Women > Tops", "img": "/get_product_picture/1", "url": "/product_details/1", "availability": "In Stock", "condition": "New" }
            const productDetail = productsConfig.find(product => product['name'] === row['name']);

            // Order: Set new order's action object with criteria value and product detail.
            orderList['order'] = Object.assign({}, row, productDetail);

            // orderTotal: Set orderTotal, if action is 'delete', set as Rs. 0 else calculate from product's price * qty.
            ...
            // Login/Signup: If this order 'loginOrSignup' is not undefined, then do login/signup at this order and get user's info to login or signup from 'userConfig.json'. 
            if(row['loginOrSignup'] !== undefined) {

                // Convert: Convert criteria value to action and username.
                // e.g. loginConfig = 'firstLogin|default' => ['firstLogin','default']
                const loginConfig = row['loginOrSignup'].split('|');

                // loginOrSignup: Set action of login or signup to order's object and main level object to check if action is signup, has to this delete account at the end of test.
                ...
                // UserInfo: Get user info from 'userConfig.json' by username and set as main level object.
            ...
            // Check Order: Set checkOrders to check order's detail after do action of this order which has to refer from latest checkOrders. 
            if(i === 0) {
                // 1st Order: 1st Order Set checkOrders and orderSummary as this order object.
                // checkOrders: Set checkOrders as this order object. 
                orderList['checkOrders'] = [orderList['order']];
                
                // orderSummary: Plus this order price to orderSummaryTotal.
                // e.g. price: "Rs. 500" => 500
                orderSummaryTotal +=  Number((orderList['order']['orderTotal']).replace('Rs. ', ''))
            } else {

                // Eles: Clone latest checkOrder from latest checkOrders in array to set new checkOrders.
                // Note: Has to use JSON.parse(JSON.stringify()); method when update new checkOrders object won't update back to latest checkOrders object. 
                ...
                // checkOrders: Set checkOrders by action.
                switch(orderList['order']['action']){
                    case 'add': 
                        ...
                        // orderSummaryTotal: Plus this product's orderTotal to orderSummaryTotal.
                        orderSummaryTotal +=  Number((orderList['order']['orderTotal']).replace('Rs. ', ''));
                        break;
                    case 'addOn':
                        // orderLineNo: Get indexOf product's order that has to update product qty and orderTotal from latestCheckOrders by 'orderNo'.
                        ...
                        // qty: Update product's qty to the same product's order in latestCheckOrders by indexOf order.
                        latestCheckOrders[orderLineNo]['qty'] = (Number(latestCheckOrders[orderLineNo]['qty']) +  Number(orderList['order']['qty'])).toString();

                        // orderTotal: Update product's orderTotal to the same product's order in latestCheckOrders by indexOf order.
                        ...
                        break;
                    case 'delete':
                        //  orderLineNo: Get indexOf  product's order that have to delete product from latestCheckOrders by 'orderNo'.
                        ...
                        // Delete: Set deleteRowNo as delete row number to click delete from table.
                        orderList['deleteRowNo'] = orderLineNo;

                        // orderSummaryTotal: Minus this product's orderTotal from orderSummaryTotal.
                        // Note: Using orderTotal from latestCheckOrders in case criteria value hasn't set qty. Update orderSummaryTotal first befor delete this order.
                        orderSummaryTotal -= Number((latestCheckOrders[orderLineNo]['orderTotal']).replace('Rs. ', '')); 
                
                        // checkOrders: Set checkOrder by removing delete this product's order out of latestCheckOrders by orderLineNo.
            ...
            // orderSummary: Set this order summary as now orderSummaryTotal.
        ...
        // orderLists: Set this test case orderLists as orderListsArray.
        testCasecriteria['orderLists'] = orderListsArray;

        // orderSummary: Set his test case orderSummary with
        // 1: checkOrders from the last checkOrder in orderListArray. 
        // 2: orderSummaryTotal as now orderSummaryTotal with currency.
        // 3: orderComment from criteria which set in the last row. Can be undefined = not do comment.
...
Cypress.Commands.add('visitPageFromClickMenu', (pageName) => {
    ...
    // If pageName is not 'video_tutorials', wait api '@homepage'.
...
Cypress.Commands.add('visitPageFromClickCategory', (categoryName) => {
    ...
    // Category: Convert categoryName for click sidebar category main title and sub title.
    ...
    // 2: Click li of category or sub title in same panel in case other panel has same category name. 
    ...
    // Checking: Checking features items's title that visit correct page. 
...
// Button: Click the button 'continue' to homepage and await api.
...
// Note: Need to set as options as criteria in case set result different from criteria.
...
Cypress.Commands.add('checkLoginMessage', (options) => {
    switch(options['result']) {
        ...
        case 'failed-incorrect':
            // failed-incorrect: Check show error message under login box.
        ...
        case 'failed-formatEmail':
            // failed-formatEmail: Checking input validation message by format email.
            checkElementValidationMessage('[data-qa="login-email"]', 'formatEmail');
            break;
    }
});
...
Cypress.Commands.add('checkSignupMessage', (result) => {
    ...
    switch(result) {
        case 'success':
            // success: If signup in 'Signup / Login' page is 'success', should to 'Signup' page and show account information form.
...
Cypress.Commands.add('filledSignupInformation', (criteria, type) => {
    ...
    // Note: Can set filledcriteria of signup from excel's criteria (test_register) or userConfig.json's criteria (test_e2eOrder).
    let filledcriteria = {};
    switch(type){
        case 'register': 
            // If type is 'register', set as criteria.
        ...
        case 'userConfig': 
            ...
            // Criteria: Set signup result as 'success' to login after create account.
            ...
            if(criteria['signup|action'] !== undefined) {
                switch(criteria['signup|action']) {
                    case 'logout':
                        ...
                        break;
                    case 'delete':
                        // Delete Account: Delete this account by clicking menu and check delete account success.
...
Cypress.Commands.add('doActionLoginOrSignup', (loginOrSignup, userInfo) => {
    ...
    // Do Action: Do action of test case e2e order which result is always 'success'.
    switch(true){
        case loginOrSignup.includes('Login'):
            // login: Login 'Signup / Login' page with userInfo & check login message.
            cy.loginWebsite({
        ...
        case loginOrSignup.includes('Signup'):
            // signup: Signup in 'Signup / Login' page with userInfo.
            cy.signupWebsite({
...
// Delete Account: Delete account by clicking menu and check delete account success.
Cypress.Commands.add('deleteAccountFromClickMenu', () => {
    // Visit: Delete account by clicking menu & Wait api delete account.
...
// User: Clear 'userType|login' cart and deleted 'userType|signup' from user account's detail in 'userConfig.json' before test e2e order test cases.
...
Cypress.Commands.add('clearUserFromUserconfig', (userName, userObject = {}) => {
    ...
    userNameArray.forEach(user => {
        ...
        // userType|signup: Clear this user order in cart info table to be empty.
        ...
        // userType|signup: If this user still can login, delete account.
    ...
    .closest('.product-image-wrapper').then($card =>{
        ...
        switch(true) {
            case order['addToCartFrom'] === undefined:
                // addToCartFrom: If undefined means test case of test_checkProducts.js
...
Cypress.Commands.add('doActionInViewDetailPage', (order, action) => {
    ...
    switch(action) {
        ...
        case 'viewCheckComment':
            ...
            // Not break to do next step if in case.
        case 'viewAdd':
        case 'viewCheckAdd':
            ...
            cy.get('.product-details .product-information').within($ele => {
                ...
                // Button: Click the button 'Add to cart', wait api and check response message that added product success.
...
Cypress.Commands.add('filledPaymentInformation', (criteria, result) => {
    ...
    // Button: Click the button 'Pay and Confirm Order'.
    ...
    switch(result) {
        ...
        case 'failed-paymentExp': 
            ...
            // Note: This website not check these error case. 
            const errorCase = {
...
Cypress.Commands.add('setBaseTestCaseResultObject', (testCaseDetail) => {
    ...
    // Note: Set testStatus default as 'Failed' then update to 'Pass' if test success. 
...
Cypress.Commands.add('writeTestCaseResultToCSV', (options) => {
    ...
    cy.task('getTestCaseResultVariable' , { name: testCaseName }).then(testCaseResult => {
        ...
        switch(type) {
            ...
            case 'register': 
            case 'e2eOrder':
                // Check test case result from all tests status have equal 'Pass'.
                expect(testStatus).to.equal('Pass', message);
                break;
            case 'visit': 
                // Check test case result from all tests status have equal 'Pass' with error messages.
